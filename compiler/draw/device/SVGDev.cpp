...
void SVGDev::rect(double x, double y, double l, double h, const char* color, const char* link)
{
    ...
    // draw the rectangle
    fprintf(fic_repr,
            "<rect x=\"%f\" y=\"%f\" width=\"%f\" height=\"%f\" rx=\"0\" ry=\"0\" style=\"stroke:#000000;fill:#FFFFFF;\"/>\n",
            x, y, l, h);//, color);
    if (link != 0 && link[0] != 0) {
...
void SVGDev::text(double x, double y, const char* name, const char* link)
{
    ...
    char name2[256];
    fprintf(fic_repr,
            "<text x=\"%f\" y=\"%f\" font-family=\"Verdana\" font-size=\"7\" text-anchor=\"middle\" "
            "fill=\"#000000\">%s</text>\n",
...
{
    char name2[256];
    fprintf(fic_repr, "<text x=\"%f\" y=\"%f\" font-family=\"Verdana\" font-size=\"7\">%s</text>\n", x, y + 2,
            xmlcode(name, name2));
}
